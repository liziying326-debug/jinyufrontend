...
// 全局计数器：解决 Instagram 渐变 id="ig" 在多实例时冲突的问题
var _socialIconCounter = 0;
...
var _socialIconSvgMap = {
  ...
  instagram: '<path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm0 2c5.514 0 10 4.486 10 10s-4.486 10-10 10S2 17.514 2 12 6.486 2 12 2zm4.8 2.6a1.6 1.6 0 100 3.2 1.6 1.6 0 000-3.2zM12 7a5 5 0 100 10 5 5 0 000-10zm0 2a3 3 0 110 6 3 3 0 010-6z"/>',
  ...
  fetch(API_BASE + '/api/social-links')
    ...
    .then(function(links) {
      ...
      el.innerHTML = enabled.map(function(link) {
        ...
        // Instagram 渐变 id 去重：为每个实例生成唯一 id
        if (link.icon === 'instagram') {
          var uniqueId = 'ig-' + (_socialIconCounter++);
          svgPath = svgPath.replace(/id="ig"/g, 'id="' + uniqueId + '"').replace(/url\(#ig\)/g, 'url(#' + uniqueId + ')');
        }
        // 确定图标颜色
